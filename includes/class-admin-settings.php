...
class AICG_Admin_Settings {
    ...
    public static function render_settings_page() {
        // 處理手動儲存火山引擎設定
        if (isset($_POST['save_volcengine_settings']) && wp_verify_nonce($_POST['_wpnonce'], 'aicg_volcengine_settings')) {
            ...
            echo '<div class="notice notice-success"><p>火山引擎設定已儲存</p></div>';
        }
        ?>
        <div class="wrap">
            <h1>AI 內容生成器設定</h1>
                    ...
                    <table class="form-table">
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <p class="description">選擇要使用的 OpenAI 模型。GPT-4 系列提供更高品質但成本較高。長文章建議使用 16K 或 Turbo 模型。</p>
                            </td>
                        </tr>
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="number" 
                                       ...
                                       min="500" 
                                       max="50000" />
                                <p class="description">文章的最少字數（建議長文章使用 GPT-4 Turbo 或 GPT-3.5 16K 模型）</p>
                            </td>
                        </tr>
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="number" 
                                       ...
                                       min="1000" 
                                       max="50000" />
                                <p class="description">文章的最多字數（可設定至 50000 字）</p>
                            </td>
                        </tr>
            ...
            <!-- 火山引擎設定的獨立表單 -->
            <div class="aicg-volcengine-section" style="margin-top: 40px; padding: 20px; background: #f0f0f0; border: 1px solid #ccc;">
                <h3>火山引擎視覺生成設定</h3>
                <p>如果上方儲存後火山引擎設定無法保存，請使用下方的獨立儲存功能：</p>
                
                <form method="post" action="">
                    <?php wp_nonce_field('aicg_volcengine_settings'); ?>
                    <table class="form-table">
                        <tr>
                            <th scope="row">
                                <label for="volcengine_access_key_id">Access Key ID</label>
                            </th>
                            <td>
                                <input type="text" 
                                       id="volcengine_access_key_id" 
                        ...
                        <tr>
                            <th scope="row">
                                <label for="volcengine_secret_access_key">Secret Access Key</label>
                            </th>
                            <td>
                                <input type="password" 
                                       id="volcengine_secret_access_key" 
                    ...
                    <p class="submit">
                        <input type="submit" name="save_volcengine_settings" class="button-primary" value="儲存火山引擎設定">
                    </p>
                </form>
            </div>
        </div>
        ...
        jQuery(document).ready(function($) {
            ...
            $('.test-api').on('click', function() {
                ...
                if (api_type === 'volcengine') {
                    var accessKeyId = $('#volcengine_access_key_id').val() || $('#aicg_volcengine_access_key_id').val();
                    var secretAccessKey = $('#volcengine_secret_access_key').val() || $('#aicg_volcengine_secret_access_key').val();
            ...
            $('#aicg_min_word_count, #aicg_max_word_count').on('change', function() {
                ...
                // 超長文章提醒
                if (max > 10000) {
                    if (!$('#long-article-notice').length) {
                        $(this).after('<p id="long-article-notice" class="description" style="color: #d63638;">⚠️ 超長文章建議使用 GPT-4 Turbo 或 GPT-3.5 16K 模型，並確保有足夠的 API 額度</p>');
                    }
                } else {
                    $('#long-article-notice').remove();
                }
            });
        });
        </script>
