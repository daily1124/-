jQuery(document).ready(function($) {
    ...
    // 抓取台灣關鍵字 - 移除 spinner，改用文字提示
    $('#aicg-fetch-taiwan-keywords').on('click', function() {
        var button = $(this);
        var originalText = button.html();
        button.text('抓取中...').prop('disabled', true);
    ...
    function showNotice(message, type) {
        ...
        var notice = $('<div class="' + noticeClass + ' aicg-notice-temp is-dismissible" style="position: fixed; top: 50px; right: 20px; z-index: 9999; max-width: 400px; box-shadow: 0 3px 6px rgba(0,0,0,0.16);"><p>' + message + '</p><button type="button" class="notice-dismiss"><span class="screen-reader-text">關閉此通知。</span></button></div>');
